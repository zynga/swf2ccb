...
package starling.display
{
    ...
    public class DisplayObjectContainer extends DisplayObject
    {
		...
		/* Is mouse/touch enabled for this item. */
		...
		/* Is mouse/touch enabled for this item's children. */
		...
		protected function getIsMouseEnabled():Boolean {
			if (!mMouseEnabled) {
				return false;
			}
			var parent:DisplayObjectContainer = this.parent;
			while (parent != null) {
				if (!parent.mMouseChildren) {
					return false;
				}
		...
		public override function releaseReferencedTextures():void {
			var len:int = mChildren.length;
			for (var i:int = 0; i < len; i++) {
				var child:DisplayObject = mChildren[i];
				child.releaseReferencedTextures();
			}
			super.releaseReferencedTextures();
		}
        ...
        public override function render(support:RenderSupport, parentAlpha:Number):void
        {
            ...
            for (var i:int=0; i<numChildren; ++i)
            {
                ...
                if (child.hasVisibleArea)
                {
					...
					if (child.mScissorsRect != null) {
						support.pushScissorRectangle(child.mScissorsRect);
