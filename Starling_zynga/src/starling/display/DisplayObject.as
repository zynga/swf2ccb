...
package starling.display
{
    ...
    public class DisplayObject extends EventDispatcher
    {
		...
		starling_internal var mX:Number;
		starling_internal var mY:Number;
		starling_internal var mPivotX:Number;
		starling_internal var mPivotY:Number;
		starling_internal var mScaleX:Number;
		starling_internal var mScaleY:Number;
		starling_internal var mSkewX:Number;
		starling_internal var mSkewY:Number;
		starling_internal var mRotation:Number;
		starling_internal var mAlpha:Number;
		starling_internal var mVisible:Boolean;
		starling_internal var mTouchable:Boolean;
		starling_internal var mBlendMode:String;
		starling_internal var mScissorsRect:Rectangle;
		starling_internal var mName:String;
		starling_internal var mUseHandCursor:Boolean;
		...
		starling_internal var mFilter:FragmentFilter;
		starling_internal var mSymbolName:String;
		starling_internal var mReferencedTextures:Vector.<Texture>;
		starling_internal var mIsDisposed:Boolean;
        ...
        public function dispose():void
        {
			if (!mIsDisposed) {
				...
				releaseReferencedTextures();
				mIsDisposed = true;
			}
        }
		
		/** True if this display object has been disposed. */
		public function get isDisposed():Boolean {
			return mIsDisposed;
		}
		
		/** Releases references to the textures this display object or it's children uses.  If a texture has 0 references it will be disposed. */
		public function releaseReferencedTextures():void {
			if (mReferencedTextures != null) {
				for each (var texture:Texture in mReferencedTextures) {
					texture.decRefCount();
				}
				mReferencedTextures.length = 0;
			}
		}
		
		/** Gets a set of textures referenced by this object and it's children. */
		public function get referencedTextures():Vector.<Texture> {
			return mReferencedTextures;
		}
        
		/** Gets a set of textures referenced by this object and it's children. */
		public function set referencedTextures(value:Vector.<Texture>):void {
			mReferencedTextures = value;
		}
		...
		/** A scissors rectangle to use when drawing this object. */
		public function getScissorsRect(rect:Rectangle):Boolean 
		{ 
			if (mScissorsRect != null) {
				rect.x = mScissorsRect.x;
				rect.y = mScissorsRect.y;
				rect.width = mScissorsRect.width;
				rect.height = mScissorsRect.height;
				return true;
			}
			return false;
		}
		
		/** Sets a scissors rect for this object.
		 * @param x     	x position of rect.
		 * @param y			y position of rect.
		 * @param width		width of rect.
		 * @param height	height of rect.
		 */
		public function setScissorsRect(x:Number, y:Number, width:Number, height:Number):void 
		{ 
			mScissorsRect = new Rectangle(x, y, width, height); 
		}
		
		/** Clears the scissors rect for this object. */
		public function clearScissorsRect():void 
		{
			mScissorsRect = null;
		}
		...
		/** The name of the asset symbol used to instantiate this object. */
		public function get symbolName():String { return mSymbolName; }
		public function set symbolName(value:String):void { mSymbolName = value; }
