...
package starling.text
{
    ...
    public class TextField extends DisplayObjectContainer
    {
        ...
        private var mNativeTextField:flash.text.TextField = new flash.text.TextField();
		...
		private function updateNativeTextField():void 
		{
			...
			if (mIsRenderedText) 
			{
				...
				mNativeTextField.defaultTextFormat = textFormat;
				mNativeTextField.width = width;
				mNativeTextField.height = height;
				mNativeTextField.antiAliasType = AntiAliasType.ADVANCED;
				mNativeTextField.selectable = mSelectable;            
				mNativeTextField.multiline = mMultiline;            
				mNativeTextField.wordWrap = mWordWrap;            
				mNativeTextField.text = mText;
				mNativeTextField.embedFonts = true;
				mNativeTextField.filters = mNativeFilters;
				
				// we try embedded fonts first, non-embedded fonts are just a fallback
				if (mNativeTextField.textWidth == 0.0 || mNativeTextField.textHeight == 0.0)
					mNativeTextField.embedFonts = false;
				
				if (mAutoScale)
					autoScaleNativeTextField(mNativeTextField);
				
			}
		}
		
		/** The native text field if this text field is rendered text. */
		public function get nativeTextField():flash.text.TextField
		{
			return mNativeTextField;
		}
        ...
        private function createRenderedContents():void
        {
			...
			var textWidth:Number  = mNativeTextField.textWidth;
			var textHeight:Number = mNativeTextField.textHeight;			
			
            var bitmapData:BitmapData = new BitmapData(width, height, true, 0x0);
            bitmapData.draw(mNativeTextField, new Matrix(1, 0, 0, 1, 0, int(yOffset)-2));
		...
		public function get textWidth():Number
		{
			if (mRequiresRedraw) redrawContents();
			if (mIsRenderedText) {
				return mNativeTextField.textWidth;
			} else {
				var r:Rectangle = this.textBounds;
				return r.width;
			}
		}
		...
		public function get textHeight():Number
		{
			if (mRequiresRedraw) redrawContents();
			if (mIsRenderedText) {
				return mNativeTextField.textHeight;
			} else {
				var r:Rectangle = this.textBounds;
				return r.height;
			}
		...
		/** Is this text field multiline. */
		public function get multiline():Boolean { return mMultiline; }
		...
		/** Is this text field word wrapped. */
		...
		/**
		 * Sets the text format for this text field.
		 *
		 * @param tf the new text format.
		 */
		public function setTextFormat(tf:TextFormat):void {
			this.fontName = tf.font;
			this.fontSize = tf.size as Number;
			this.bold = tf.bold as Boolean;
			this.italic = tf.italic as Boolean;
			this.underline = tf.underline as Boolean;
			this.color = tf.color as uint;
			this.kerning = tf.kerning as Boolean;
		}

		/**
		 * Gets the text format for this text field.
		 * @return the text format.
		 */
		public function getTextFormat():TextFormat {
			var tf:TextFormat = new TextFormat();
			tf.font = this.fontName;
			tf.size = this.fontSize;
			tf.bold = this.bold;
			tf.italic = this.italic;
			tf.underline = this.underline;
			tf.color = this.color;
			tf.kerning = this.kerning;
			return tf;
		}
		...
		public function get editable():Boolean { return mSelectable && mNativeTextField.type == TextFieldType.INPUT; }
		public function set editable(sel:Boolean):void {
			if (sel) {
				mNativeTextField.type = TextFieldType.INPUT;
			}
			else {
				mNativeTextField.type = TextFieldType.DYNAMIC;
			}
		...
		/**
		 * When touched, displays the flash textfield if editable
		 */
		private function onTouch(event:TouchEvent):void
		{
			for each (var touch:Touch in event.touches) {
				if (touch.phase == "began" && editable) {
					var curTf:starling.text.TextField = starling.display.Stage.sFocusedTextfield;
					if (curTf != this)
					{
						var flashStage:Stage = Starling.current.nativeStage;
						
						if (curTf != null) {
							flashStage.removeChild(curTf.nativeTextField);
							curTf.text = curTf.nativeTextField.text;
							curTf.visible = true;
						}
						var bounds:Rectangle = getBounds(stage);
						
						nativeTextField.x = bounds.x;
						nativeTextField.y = bounds.y-2; //hack by Edison. The y coord is usually around 2 px off, not sure why
						nativeTextField.setTextFormat(this.getTextFormat());
						
						visible = false;
						flashStage.addChild(nativeTextField);
						
						starling.display.Stage.sFocusedTextfield = this;
					}
				}
			}
		}
