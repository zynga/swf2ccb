...
package starling.textures
{
    ...
    public class TextureAtlas
    {
        ...
        /** Create a texture atlas from a texture by parsing the regions from an XML or JSON file. */
        public function TextureAtlas(texture:Texture, atlasData:Object=null)
        {
            mTextureRegions = new Dictionary();
            mTextureFrames  = new Dictionary();
            mAtlasTexture   = texture;
			
			texture.textureAtlas = this;
            
            if (atlasData is XML)
                parseAtlasXml(atlasData as XML);
			else if (atlasData != null) 
				parseAtlasJson(atlasData);
        }
        ...
        /** This function is called by the constructor and will parse a JSON in Starling's 
         *  default atlas file format. Override this method to create custom parsing logic
         *  (e.g. to support a different file format). */                
		protected function parseAtlasJson(atlasJson:Object):void
		{
			var scale:Number = mAtlasTexture.scale;
			
			var subTex:Array = atlasJson["subTexture"];
			
			for each (var subTexture:Object in subTex)
			{
				var name:String        = subTexture["name"];
				var x:Number           = subTexture["x"] / scale;
				var y:Number           = subTexture["y"] / scale;
				var width:Number       = subTexture["width"] / scale;
				var height:Number      = subTexture["height"] / scale;
				var frameX:Number      = subTexture["frameX"] / scale;
				var frameY:Number      = subTexture["frameY"] / scale;
				var frameWidth:Number  = subTexture["frameWidth"] / scale;
				var frameHeight:Number = subTexture["frameHeight"] / scale;
